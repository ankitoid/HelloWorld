  ...
  return (
    <div className="flex flex-col gap-8 bg-blueCustomColor text-white p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:text-lg">
        ...
        <ThoughtCard
          title="Enhancing Broadcasting with Optimized Cloud Servers"
...
function ThoughtCard({
  ...
  return (
    ...
    >
      ...
      {/* Image blur effect applied only on hover */}
      <div
      ...
      <div className="relative z-10 flex flex-col justify-between h-full p-4">
        ...
        {buttonText && (
          <button className="bg-greenCustomColor hover:bg-greenCustomColor2 text-white font-bold py-2 px-4 rounded-full self-center mb-12">
            {buttonText}
          </button>
        )}
        ...
        {hoverEffect && (
          <div className="transform translate-y-full group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-700 ease-in-out mt-auto">
            <p className={`text-${textColor} mb-8 font-semibold`}>{description}</p>
          </div>
        )}
      </div>
    </div>
